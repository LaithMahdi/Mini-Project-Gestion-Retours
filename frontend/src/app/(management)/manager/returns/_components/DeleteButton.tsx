...
const DeleteButton = ({ data, open, onOpenChange }: Props) => {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: () => {
      return apiClient.delete(`${RETURNS_ENDPOINT}/delete/${data.id}`);
    },
    onSuccess: () => {
      toast.success("Retour supprime avec succes.");
      queryClient.invalidateQueries({ queryKey: [RETURNS_KEY] });
      onOpenChange(false);
    },
    onError: (error) => {
      toast.error("Erreur lors de la suppression du retour.");
      ...
      <AlertDialogContent className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="text-slate-600 dark:text-slate-300">
            Cette action est irreversible. Le retour de &nbsp;
