...
@ApiResponses(value = {
        @ApiResponse(
                responseCode = "400",
                description = "Bad Request - Invalid registration/login payload",
                content = @Content(
        ...
        @ApiResponse(
                ...
                content = @Content(
                        schema = @Schema(implementation = ApiErrorResponse.class),
                        examples = @ExampleObject(value = "{\"success\":false,\"message\":\"Invalid email or password\",\"data\":null}")
                )
        ),
        @ApiResponse(
                responseCode = "500",
...
public @interface AuthApiErrorResponses {
}
